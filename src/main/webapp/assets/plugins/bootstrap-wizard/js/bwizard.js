...
$.widget("bootstrap.bwizard", {
	...
	_createButtons: function () { 
		...
		if (!this.buttons) {
			...
			if(nextBtnText != ''){
				this.nextBtn =
					$("<li class='next'><a href='#'>" +	nextBtnText + "</a>").appendTo(this.buttons).bind({
						'click': function () {

							if (self.number==null){
								self.number=1;
							}
							var nomQuest = $("#controlador").val().toString();
							if(nomQuest == "c2"){// controla que solo se 
								var p; // contador que cuenta que todos los inputs sumen 10
								switch(self.number) {
									case 1:
										var p1 = parseInt($("#p1").val()); // asigno a la var p1 el valor del input que tiene el id "#p1
										...
										var p9 = parseInt($("#p9").val());
										var p10 = parseInt($("#p10").val());
									break;
									case 2:
										p1 = parseInt($("#p11").val()); 
										p2 = parseInt($("#p12").val());
									...
									case 7:							
										...
										p8 = parseInt($("#p68").val());
										p9 = parseInt($("#p69").val());
										p10 = parseInt($("#p70").val());
									break;	
								}

								p=(p1+p2+p3+p4+p5+p6+p7+p8+p9+p10); // sumo en p todos los valores de la parte en la que estamos
									
								if (p!=10){
									alert("lo sentimos, sus respuestas no suman 10, revise esta parte del cuestionario");
								}else if (p==10){
									self.next();
									self.number= self.number+1;
									return false;
